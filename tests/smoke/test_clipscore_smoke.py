...
def test_clipscore_smoke():
    from aesthetic_scoring import score_clipscore
    from aesthetic_scoring.clipscore import unload, _load
    ...
    result = score_clipscore([PHOTO_A, PHOTO_B], PROMPT)
    ...
    if is_cuda:
        assert elapsed < 60, f"CLIPScore smoke took {elapsed:.1f}s, expected < 60s (CUDA)"
        assert peak_gib < 5.5, f"Peak VRAM {peak_gib:.2f} GiB exceeded 5.5 GiB"
    elif is_mps:
        assert elapsed < 180, f"CLIPScore smoke took {elapsed:.1f}s, expected < 180s (MPS)"
    ...
    print(f"\nCLIPScore scores: {result.scores}, ranked: {result.ranked_image_ids}, "
